...
const [statusFilter, setStatusFilter] = useState("All");
  ...
  const handleDownload = async () => {
    try {
      const response = await api.get("/caretaker/download-students", { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'hostel_students.xlsx');
  ...
  const handleExcelUpload = async (e) => {
    ...
    try {
      await api.post("/caretaker/upload-rooms", formData);
  ...
  // Logic: Room 302 -> Floor 3
  const studentFloor = student.roomNumber ? Math.floor(parseInt(student.roomNumber) / 100).toString() : "None";
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Room</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredStudents.length > 0 ? (
                  ...
                  <tr key={student._id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] font-black text-indigo-500 uppercase tracking-widest">Degree - {student.degree || "B.Tech"}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-6 text-center">
                      {student.roomNumber ? (
                        ...
                        <span className="text-[10px] font-black text-amber-500 uppercase px-3 py-1 bg-amber-50 rounded-full border border-amber-100 animate-pulse">
                          Unassigned
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-8 py-6 text-right">
                      <button className="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black uppercase hover:bg-slate-800 hover:text-white transition-all shadow-sm">
                        Details
                      </button>
                    </td>
                  </tr>
                ...
                <tr>
                  <td colSpan="5" className="px-8 py-20 text-center bg-white rounded-b-[2.5rem]">
